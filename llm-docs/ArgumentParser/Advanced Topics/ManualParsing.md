...
Let's implement the `Select` command discussed in [Validation](Validation.md), but using a scripty style instead of the typical command. First, we define the options as a `ParsableArguments` type:
...
Let's see how this works by using the `Math` command and subcommands defined in [CommandsAndSubcommands](CommandsAndSubcommands.md). This time, instead of calling `Math.main()`, we'll call `Math.parseAsRoot()`, and switch over the result:
